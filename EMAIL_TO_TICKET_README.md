...
- **`POST /api/cron/process-email`** – connects to **both** mailboxes via IMAP, fetches unread emails, creates tickets (open or pending) or adds comments, marks processed.
- Poll every **30–60 seconds**. Secure with **`CRON_SECRET`**.
